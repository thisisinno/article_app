...
describe("frontend architecture",()=>{
  ...
  it("guards cancellable route requests while preserving retry UI",()=>{for(const path of ["app/post/[postId]/page.tsx","app/notifications/page.tsx","app/profile/[username]/page.tsx"]){const page=readFileSync(join(root,path),"utf8");expect(page).toContain("isActiveRequest");expect(page).toContain("isRequestCancellation");expect(page).toContain("Retry")}const post=readFileSync(join(root,"app/post/[postId]/page.tsx"),"utf8");expect(post).toContain("setPost(null)");expect(post).toContain(".finally(")});
