  ...
  if (!response.ok) {
    ...
    const messages: Record<string, string> = {
      ...
      backend_gateway_error:
        "The application server is temporarily unavailable. Please retry.",
      server_error:
        "The application server could not complete the request. Please retry.",
    ...
    throw new ApiError(
      messages[code] || details.message || fallback,
      response.status,
      code,
...
export async function getCsrfToken({
  ...
  if (!csrfPromise) {
    csrfPromise = request<{ csrfToken: string }>(`${API_BASE}/auth/csrf/`)
      .then((data) => {
        if (!data?.csrfToken)
          throw new ApiError(INVALID_MESSAGE, 200, "invalid_response", true);
...
export async function api<T>(
  ...
  if (!path.startsWith("/") || path.startsWith("//"))
    throw new ApiError("Invalid API path.", 0, "invalid_path", false);
...
export function validateFeedResponse(value: unknown) {
  if (
    ...
    throw new ApiError(
      "The feed server returned an invalid response.",
      502,
      ...
      media: Array.isArray(item.media)
        ? item.media.filter(
            (media) =>
              ...
              typeof media.sort_order === "number",
          )
        : [],
    }));
  return { ...value, results } as {
    results: import("./types").Post[];
...
export function validateCommentsResponse(value: unknown) {
  if (
    ...
    throw new ApiError(
      "The comments server returned an invalid response.",
      502,
      "invalid_comments_response",
      true,
    );
  }
  return value as {
    results: import("./types").Comment[];
    next_cursor?: string | null;
    total_count?: number;
  };
}
